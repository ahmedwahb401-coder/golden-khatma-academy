...
export default async function AdminStudentsPage() {
  ...
  const students =
    data?.map((student: any) => ({
      ...student,
      linked_user_name: student.users?.full_name ?? null,
    })) || [];
  ...
  const linkedIds = new Set(
    students.map((s: any) => s.user_id).filter(Boolean)
  );

  const availableUsers = (studentUsers || []).filter(
    (u) => !linkedIds.has(u.id)
  );
    ...
    <div className="p-6 md:p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm opacity-60 mt-1">
            {students.length} طلب تسجيل إجمالاً
          </p>
        </div>
      </div>
      ...
      {error && (
        <div className="rounded-xl border border-red-500/20 bg-red-500/5 p-4 text-sm text-red-600 mb-5">
          تعذّر تحميل بيانات الطلاب. تأكد من ضبط متغيرات Supabase في
          .env.local.
        </div>
      )}
